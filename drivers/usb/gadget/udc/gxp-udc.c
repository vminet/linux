				 ...
				 int size)
{
	...
	//printk("gxp_udc_ep_init() ep%d evdepcc = 0x%04x\n", ep->epnum, val);
								...
								gfp_t gfp_flags)
{
	...
	req = kzalloc(sizeof(struct gxp_udc_req), gfp_flags);
	if (!req)
		return NULL;
